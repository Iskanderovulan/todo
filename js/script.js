...
const renderTodos = () => {
    ...
    todos.map(todo => {
        ...
        date.textContent = `
        Todo was created ${current_date.getDate()}.${current_date.getMonth() + 1}.${current_date.getFullYear()} в ${current_date.getHours()} часа(-ов) ${current_date.getMinutes()} минут(-ы)
        `
        ...
        buttons.append(done, editName, editDescr, del)
        block.append(mess, description, date, doneIsComplete, buttons)
        output.append(block)
        row.append(output)
    })
}
